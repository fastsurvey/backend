...
def generate_form_data(form_data):
    return render_template("survey_3_email.html", election=form_data["election"])

def generate_change_url(form_data):
    change_url = f"{FRONTEND_URL}fvv-ss20-go/form?email={form_data['email']}"
...
def generate_verify_url(verification_token):
    return f"{BACKEND_URL}fvv-ss20-go/verify/{verification_token}"
